class IssuesController < ApplicationController
  ...
  def filtered_issues
    issues = if params[:project] == "ALL" || params[:project].nil?
               @accessible_projects.collect(&:issues).flatten
             else
               project = nil
               @accessible_projects.each { |ap| project = ap if ap.ProjectName == params[:project] }
               project.issues
             end

    issues.delete(nil) if issues
    issues
  end
  ...
  def create
    ...
    @issue = Issue.new(params[:issues])

    if @issue.save
      project = Project.find_by_ProjectName(params[:issues][:Project])
      project.issues << @issue
      project.save
      # TODO: move emails to bg job
  ...
  def edit
    @object_issues = Issue.find_by_objectId(params[:id])
  ...
  def destroy
    issue = Issue.find_by_objectId(params[:id])
  ...
  def query project
    ...
    if project.present?
      query.merge!(:Project => project[0])
      query.merge!(:Project => project[1])
    end
    query
  end
